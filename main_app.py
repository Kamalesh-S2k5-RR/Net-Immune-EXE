...
class DashboardWindow:
    ...
    def manual_file_scan(self):
        ...
        if file_path:
            ...
            def run_scan():
                try:
                    # 1. Read from the ORIGINAL file directly (No copying!)
                    try:
                        ...
                        content_peek = f"[Error Reading Original File: {e}]"

                    # 2. Create a tiny 1KB text marker instead of copying the 6GB file!
                    marker_path = os.path.join(WATCH_FOLDER, filename + ".scanned.txt")
                    try:
                        with open(marker_path, "w", encoding="utf-8") as marker_file:
                            marker_file.write(f"Original File: {file_path}\nScanned On: {time.strftime('%Y-%m-%d %H:%M:%S')}\nStatus: Processed")
                    except: pass
                    
                except Exception as main_e:
                    content_peek = f"[System Error: {main_e}]"
